...
export class CrawlingConstller {
  constructor() {}

  async getData(req: Request, res: Response, next: NextFunction) {
    let page = req.params.page ?? 1;
    let url: string =
      "https://shopping.naver.com/window/api/graphql?operationName=getPagedProducts&variables=%7B%22isIncludeProductDetail%22%3Afalse%2C%22params%22%3A%7B%22page%22%3A" +
    ...
    const headers = {
      referer:
        "https://shopping.naver.com/window/brand-pet/category?unionCategory=CAT",
      "user-agent":
    ...
    try {
      ...
      if (responseData) {
        for (let i = 0; i <= resData.length - 1; i++) {
          chid.push(
            responseData["data"]["pagedProducts"]["products"][i]["product"][
              "channel"
            ]["id"]
          );
        }
    ...
    //   }
      ...
      const channelUrl: string =
      "https://shopping.naver.com/window-products/brandpet/5470708974?NaPm=ct%3Dllcbasgu%7Cci%3Dshoppingwindow%7Ctr%3Dpetcat%7Chk%3D5c9856ec283047deac75c40e6a11eabfab5f0f08%7Ctrx%3D&topDetailVideo=false";
...
export const crawlingConstller = new CrawlingConstller();
export const catGetData = crawlingConstller.getData.bind(CrawlingConstller);
